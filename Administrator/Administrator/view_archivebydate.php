            ...
            <legend class="scheduler-border">Archived</legend>
                                    ...
                                    <li class="active">
                                        ...
                                        <ul class="collapse list-unstyled" id="MyFile">
                                           <?php 
                                                ...
                                                require_once("../config/connection/access_db.php");
                                                $id = $conn->real_escape_string(trim($_SESSION['Username']));
                                                $sql = $conn->query("SELECT *, count(dates) as countdate FROM personal_file group by dates") or die ($conn->error());
                                            ...
                                            <li>
                                                <a href="view_archivebydate.php?PersonalFileID=<?php echo htmlentities($row['PersonalFileID']);?>"><?php echo date('M-d-Y', strtotime($date_created));?> (<?php echo $count;?>)</a>
                                            </li>
                                             <?php } ?>
                                        </ul>
                                    </li>
                ...
                <legend class="scheduler-border">Archive File Records</legend>
                    <div class="table-responsive table-bordered">
                        <table id="home_table" class="display" cellspacing="0" width="100%">
                            <thead>
                                <tr>
                                    <th>File Name</th>
                                    <th>Size</th>
...
<!--                                     <th>Size</th>
                        ...
                        <?php 
                                ...
                                $sql = $conn->query("SELECT  * FROM personal_file where dates = '$id'") or die ($conn->error());
                                  while($row = $sql->fetch_array()){ 
...
<!--     <script>
        $(document).ready(function() {
            $('#home_table').DataTable({
                ...
                "ajax": "../controllers/fetchcabinet_process.php",
